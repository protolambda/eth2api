...
func Peer(ctx context.Context, cli eth2api.Client, peerId eth2api.ApiPeerId, dest *eth2api.Peer) (exists bool, err error) {
	return eth2api.SimpleRequest(ctx, cli, eth2api.FmtGET("eth/v1/node/peers/%s", peerId), dest)
}

// Retrieves number of known peers.
func PeerCount(ctx context.Context, cli eth2api.Client, dest *eth2api.PeerCountResponse) error {
	return eth2api.MinimalRequest(ctx, cli, eth2api.PlainGET("eth/v1/node/peer_count"), dest)
}
...
func Peers(ctx context.Context, cli eth2api.Client, state []eth2api.ConnectionState, direction []eth2api.ConnectionDirection, dest *[]eth2api.Peer) (exists bool, err error) {
	var q eth2api.Query
	...
	return eth2api.SimpleRequest(ctx, cli, eth2api.QueryGET(q, "eth/v1/node/peers"), dest)
}
